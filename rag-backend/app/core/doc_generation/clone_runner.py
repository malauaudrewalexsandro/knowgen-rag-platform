...
from app.core.doc_generation.document_blocks import detect_title, extract_document_blocks
...
def run_clone(document_id: str, format: str, title: str | None = None) -> tuple[str, str]:
    ...
    # Prefer a real detected title (e.g. the resume owner's name, if the
    # document opens with a top-level heading) over the raw uploaded
    # filename — falls back to the filename when no such heading exists.
    resolved_title, blocks = (title, blocks) if title else detect_title(blocks, doc["filename"])
